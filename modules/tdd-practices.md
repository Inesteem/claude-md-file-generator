...
- Use pytest fixtures for shared setup; prefer function-scoped fixtures unless state is truly shareable
- Use `@pytest.mark.parametrize` for data-driven cases instead of loops inside tests
- Handle optional dependencies with `@pytest.mark.skipif` rather than silent no-ops
- Label tests so CI can run unit tests fast and integration tests on a separate schedule
- Guard against CI environment differences explicitly: use `tmp_path` over hardcoded temp dirs, avoid assuming specific ports are available
